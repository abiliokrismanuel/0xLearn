...
install_ha_master() {
  ...
  sed -i "s#server: https://.*:6443#server: https://${NLB_ADDRESS}:6443#" ~/.kube/config
...
join_worker() {
  ...
  sudo chown $(id -u):$(id -g) ~/.kube/config || true
  sed -i "s#server: https://.*:6443#server: https://${NLB_ADDRESS}:6443#" ~/.kube/config || true
}
